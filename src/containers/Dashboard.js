...
class Dashboard extends Component {
  ...
  addMemberHandler(userID, reqId) {
    ...
    axios
      ...
      .then(response => {
        ...
        window.location.href = "/dashboard?id=" + this.state.project._id;
      })
      .catch(error => {
        this.setState({
          error: true,
  ...
  deleteProjecthandler() {
    axios
      ...
      .then(response => {
        window.location.href = "/projects?id=" + this.state.userId._id;
      })
      .catch(error => {
        this.setState({
          error: true,
  ...
  deleteMemberHandler(userId) {
    axios
      .request({
        method: "delete",
        url: "/api/projects/member/" + this.state.project._id + "/" + userId
      })
      .then(response => {
        window.location.href = "/dashboard?id=" + this.state.project._id;
      })
  ...
  openEditModal = e => {
    ...
    ModalManager.open(
      <EditProject
        ...
        onRequestClose={() => true}
        onDeleteMember={this.deleteMemberHandler.bind(this)}
        {...this.props}
      />
    );
    e.preventDefault();
  ...
  openDeleteModal = e => {
    ...
    ModalManager.open(
      <DeleteProject
        style={modalStyle}
        data={this.state.projectId}
        onDelete={this.deleteProjecthandler.bind(this)}
        onRequestClose={() => true}
        {...this.props}
      />
    );
                  ...
                  {this.state.memberNames.map((members, index) => {
                    return (
                      <div key={index}>
                        ...
                        {" " + this.memberNames[index].fullName}
                      </div>
                    );
                  })}
                  {adminAccess}
