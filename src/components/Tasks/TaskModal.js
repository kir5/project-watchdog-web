...
class TaskModal extends PureComponent {
  ...
  render() {
    ...
    let allUsers, final, notifyDeadlineError, attachedFiles;
    this.state.completed === true ? (final = true) : (final = false);
    if (this.props.files.length > 0) {
      attachedFiles = this.props.files.map((file, index) => {
        return (
          <div key={index}>
            <a href={file}>{file}</a>
          </div>
